...
function marstechcoder_send_email(){
    ...
    if (isset( $_POST["marstechcoder_send_email_nonce"] ) && wp_verify_nonce($_POST['marstechcoder_send_email_nonce'], 'marstechcoder_send_email_nonce')) {
        ...
        $user_calf_right   =  $_POST['user_calf_right'];
        $user_calfs        =  $_POST['user_calfs'];
        $user_quads_right  = $_POST['user_quads_right'];
        $weight            =  $_POST['weight'];
...
Quads: '.$user_quads .'<br/>Calfs Left : '.$user_calfs .'<br/>Calfs Right: '.$user_calf_right .'<br/> Quads Right: '.$user_quads_right .' ';
...
//                print_r($_POST);exit;
        ...
        $fitness_data_insert =
            array(
                ...
                'user_quads_right'=> $user_quads_right,
                'user_calfs' => $user_calfs,
                'user_calf_right'=> $user_calf_right,
                'user_id' => $user_id,
                'created_data' => $date,
                'status' => $status,
                'weight' => $weight,
...
//        $insertFitness = INSERT INTO `wp_fitness_measurements`(`id`, `user_chest`, `user_biceps`, `user_waist`, `user_hips`, `user_quads`, `user_calfs`, `user_calf_right`, `user_quads_right`, `weight`, `user_id`, `created_data`, `status`) VALUES ($user_chest,$user_biceps,$user_waist,$user_hips,$user_quads,$user_calfs,$user_calfs_right,$user_quads_right,$weight,$user_id,$status);



        $table_name = 'wp_fitness_measurements';
        $result_fitness = $wpdb->insert($table_name,$fitness_data_insert);

//        print_r($result_fitness);exit;
        if ($result_fitness){
                                    ...
                                    <div class="column one">
                                        ...
                                        <form role="form" action="" method="post" enctype='multipart/form-data'>
                                            ...
                                            <fieldset>
                                                ...
                                                <div class="column one-second rgistr-frm">
                                                    <label for="user_quads_right"><?php _e('Quads Right'); ?></label>
                                                ...
                                                <div class="column one-second rgistr-frm">
                                                    <label for="user_calf_right"><?php _e('Calfs Right'); ?></label>
                                                    <input name="user_calf_right" id="user_calf_right" type="text" class="required" value="<?php echo $_POST['user_calf_right']; ?>"/>
                                                </div>
                                                <div class="column one-second rgistr-frm">
...
function fitness_form(){
    if(isset($_POST['submit'])){
        ...
        $user_quads_right = $wpdb->escape($_POST['user_quads_right']);
        $user_calfs = $wpdb->escape($_POST['user_calfs']);
        $user_calf_right = $wpdb->escape($_POST['user_calf_right']);
